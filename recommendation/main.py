    ...
    for i in range(len(data)):
        ...
        for j in list_distance:
            ...
            if hs < radius and data[i]['latitude'] != j[1] and data[i]['longitude'] != j[0]:
                if data[i] not in data_hs_nodis:
                    hs = format(hs, '.2f')
                    d = dict(data[i].items())
                    d["distance"] = float(hs)
...
async def rating_home(user_id: int):
    ...
    location = locations.select().where(
        locations.c.locationId.in_(user_final_ratings[:10]))
    data_location = await database.fetch_all(location)
    # order_category = []
    final_data = []

    for i in data_location:
        if i["category"] == 1 and i not in final_data:
            final_data.append(i)

    # for i in final_data:
    #     print("data = ",i["locationId"])
...
async def rating_database(user_id: int):
    ...
    #     for j in data:
    #         if j["locationId"] == i and j not in final_data:
    #             final_data.append(j)
    # for i in data:
    #     print("data = ",i["locationId"])
...
async def get_Type_location(user_id: int):
    ...
    Dict_activity = {'บันจี้จัมป์': 1, 'ปีนเขา': 1, 'ปาร์ตี้': 2, 'ดูพระอาทิตย์ขึ้น-ตก': 3,
                     'ดูทะเลหมอก': 3, 'ล่องแก่ง': 4, 'ล่องเรือ': 4, 'ดูสวนดอกไม้': 5, 'ดูงานศิลปะ': 6,
...
def decision_tree(input_activity):
    data = load(
        'C:/Users/User/Desktop/TripPlanner/recommendation/decision_tree_model.joblib')
